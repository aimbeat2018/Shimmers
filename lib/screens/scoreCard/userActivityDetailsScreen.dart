...
class UserActivityDetailsScreen extends StatefulWidget{
  ...
  UserActivityDetailsScreen({required this.userId, required this.fromDate, required this.toDate,required this.activityType});
    ...
    if (mounted) {
      Future.delayed(Duration.zero, () async {
        Get.find<ScoreController>()
            .employeeactivityDetails(user_id:widget.userId,activityType:widget.activityType,fromDate: widget.fromDate,toDate: widget.toDate);
      });
    }
  ...
  Widget build(BuildContext context) {
    ...
    GetBuilder<ScoreController>(builder: (scoreController)
    {
      return Scaffold(
        ...
        body: scoreController.isLoading ?Center(child: CircularProgressIndicator()):
        scoreController.employeeActivityDetail!.data==null||scoreController.employeeActivityDetail!.data!.isEmpty?
        Center(
            child: SizedBox(
